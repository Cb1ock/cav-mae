...
def process_audio(input_f):
    base_path = "/data/public_datasets/MAFW/data/audio"
    ...
    os.system('ffmpeg -i {:s} -ar 16000 {:s}'.format(input_f, output_f_1)) # resample audio to 16000 Hz
...
parser.add_argument("-input_file_list", type=str, default='egs/MAFW/mafw_audio_list.csv', help="Should be a csv file of a single columns, each row is the input audio path.")
parser.add_argument("-target_fold", type=str, default='/data/chenghao/MAFW/data/sample_audio/', help="The place to store the processed audio files.")
